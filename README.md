...
| ScyllaDB | Регулярные бэкапы (snapshots + incremental backup в S3/GCS) <br> Replication Factor (RF=3) – данные реплицируются на 3+ узла, чтобы пережить отказ одного или даже двух - Multi-DC синхронизация |
