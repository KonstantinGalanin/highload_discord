...
&nbsp;&nbsp;&nbsp;&nbsp;Будем использовать **Google Maglev**, внутри него используется VIP(virtual IP), поэтому при падении одного сервера, трафик перенаправится на другой сервер с тем же VIP, и звонок продолжится. Упавшие сервера будем мониторить с помощью keepalived.

#### Локальная балансировка текстового чата

&nbsp;&nbsp;&nbsp;&nbsp; Для локальной балансировки будем использовать Nginx в  качестве reverse-proxy, который  будет принимать запросы пользователей и направлять их на нужные сервисы в кластере Kubernetes.
