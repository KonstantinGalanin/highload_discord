...
## Оглавление  
- <a href="#6.-Физическая-схема-БД">6. Физическая схема БД</a>  
...
| ---------------------- | -------------------------: | -------------------------: |
| servers |                  Хранит информацию о сообществах | |
| server_users             | Связывает пользователей с сообществом        |
| channels                  |               Хранит информацию о каналах внутри сообществ | 
| users |                 Хранит информацию о пользователях |   
| channel_messages                  |   Хранит сообщения в каналах |
| attachments                  |   Хранит вложения (файлы), прикрепленные к сообщениям |
| voice_members                  |   Связывает пользователя с голосовым каналом |
...
| ---------------------- | -------------------------: | -------------------------: |
| servers |                  PostgreSQL | id, title|
| server_users             | PostgreSQL        | server_id, user_id|
| channels                  |               PostgreSQL | server_id |
| users |                 PostgreSQL |  id, username |
| channel_messages                  |   ScyllaDB | username, сreated_at |
...
&nbsp;&nbsp;&nbsp;&nbsp;Партиционировать будем по **channel_id** и **bucket** - окно времени, в нашем случае 1 минута, чтобы в партиции не было слишком много данных  
&nbsp;&nbsp;&nbsp;&nbsp;*Так как RPS на запись 9838 => в минуту 590,280 записей в БД*
